...
public class AOPUtility {
	...
	public static boolean isEligibleToAspectProcess(Object bean, ArrayList<Object> aspects) {
		for (Object aspect : aspects) {
			for (Method m : aspect.getClass().getDeclaredMethods()) {
				//We should add the others : After, Around
				if (m.isAnnotationPresent(Before.class)||m.isAnnotationPresent(After.class)
					...
					for (Method beanMethod : bean.getClass().getDeclaredMethods()) {
						try {
							if(checkExpressionMatching(beanMethod,
									getExpressionValueFromAnnotation(m)))
								return true;
					...
					if(expression.contains(AND_STRING)) {
						...
						for(String exp:expressions) {
							...
							if(exp.contains(AT_STRING)) {
								isEligible  = isEligible && evaluateAnnotationsMatching(rightLeftTrimForSingleWord(exp), getMethodAnnotations(m));
							}
							else isEligible = isEligible && evaluateExpression(buildMethodSignature(m), rightLeftTrimForSingleWord(exp));
						}
						return isEligible;
					}
					else {
			...
			if (strM.equals(strE))
				...
				if(methodSignature.contains("throws")) {
					...
					if(expression.contains(RIGHT_BRACKET_STRING)){
						...
						if(!methodException.equals(expressionException) && !expressionException.contains(VERSATILE_VALUE_STRING)) 
							return false;
					}
				}
				//
				if (strE.contains(VERSATILE_VALUE_STRING)) {
					...
					for (int i = (Arrays.asList(strEArray)).size(); i > 0; i--) {
						if (!strEList.get(i - 1).equals(VERSATILE_VALUE_STRING) && !strEList.get(i - 1).equals(subMList.get(i - 1)))
							...
							if (strEList.get(i - 1).equals(subMList.get(i - 1)) || strEList.get(i - 1).equals(VERSATILE_VALUE_STRING)) {
								strEList.remove(i - 1);
								subMList.remove(i - 1);
							}
							if (strEList.size() == 0)
	...
	private static boolean checkParametersMatching(String methodSignature, String expression) {
		if (expression.contains("(..)")) return true;
			...
			if(methodSignature.contains(DOUBLE_BRACKET_STRING)) return true;
			else return false;
		}
		else {
			String [] expParametersArray=(expression.substring(expression.indexOf(LEFT_BRACKET_STRING)+1, expression.lastIndexOf(RIGHT_BRACKET_STRING))).split(COMMA_STRING);
			ArrayList<String> expParametersList = new ArrayList<>(Arrays.asList((expression.substring(expression.indexOf(LEFT_BRACKET_STRING)+1, expression.lastIndexOf(RIGHT_BRACKET_STRING))).split(COMMA_STRING)));
			ArrayList<String> methParametersList = new ArrayList<>(Arrays.asList((methodSignature.substring(methodSignature.indexOf(LEFT_BRACKET_STRING)+1, methodSignature.lastIndexOf(RIGHT_BRACKET_STRING))).split(COMMA_STRING)));
			...
			if (expParametersList.containsAll(methParametersList))
				...
				for (int i = expParametersArray.length; i > 0; i--) {
                     if(expParametersList.get(i-1).equals(VERSATILE_VALUE_STRING)|| (expParametersList.get(i-1).equals(methParametersList.get(i-1)))) {
                    	 expParametersList.remove(i-1);
                    	 methParametersList.remove(i-1);
                     }
				}
				if(expParametersList.size()==0) return true;
				else return false;
			}
		}
	}
	...
	public static String buildMethodSignature(Method m) {
		...
		/*StringBuilder signatureString = new StringBuilder(Modifier.toString(m.getModifiers()) + SPACE_STRING + m);
		signatureString.append(LEFT_BRACKET_STRING);
		for (String parameterType : parmetersType) {
			signatureString.append(parameterType + COMMA_STRING);
		}
		signatureString.delete(signatureString.length() - 1, signatureString.length());
		signatureString.append(RIGHT_BRACKET_STRING);
	...
	public static ArrayList<String> getMethodAnnotations(Method m) {
		...
		for(Annotation an: m.getAnnotations()) {
			String annotationName= "@annotation"+LEFT_BRACKET_STRING+an.getClass().getSimpleName()+RIGHT_BRACKET_STRING;
			annotationsList.add(annotationName);
		}
    ...
    private static boolean evaluateAnnotationsMatching(String expression, ArrayList<String> annotations) {
    	if(expression.contains(AT_STRING)) {
    		String annotationName=expression.substring(expression.indexOf(AT_STRING), expression.indexOf(RIGHT_BRACKET_STRING)+1);
    		if(annotations.contains(annotationName)) return true;
    	}
    	return false;
    }
	public static void main(String[] args) {
		Method m = null;
		try {
			m= ObjectDAOImpl.class.getMethod("aspectTest");
		} catch (NoSuchMethodException | SecurityException e) {
			e.printStackTrace();
		}
		/*System.out.println(getMethodAnnotations(m));
		System.out.println(evaluateAnnotationsMatching("@annotation(Override)", getMethodAnnotations(m)));*/
		System.out.println(buildMethodSignature(m));
        //public void com.venus.test.dao.ObjectDAOImpl.aspectTest()

		try {
			System.out.println(checkExpressionMatching(m, "* * *.ObjectDAOImpl.aspectTest() || @annotation(Override)"));
		} catch (VenusAOPFormatNotSupported e) {
			e.printStackTrace();
		}

		//System.out.println(AOPUtility.evaluateExpression(
				//"public com.fsb.aop.French.saluer(String , int) throws exception", "public *.fsb.aop.*.saluer(String, *) *"));
	}
}
